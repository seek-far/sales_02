...
// base64-in-JSON, so there's no ~33% inflation and no giant string to build —
// large recordings save much faster. The progress events also stop the panel
// from looking frozen during a multi-minute upload (which used to make users
// re-click and launch concurrent coach-upload runs).
function uploadAudioRaw(file) {
  return new Promise((resolve, reject) => {
    const xhr = new XMLHttpRequest();
    xhr.open("POST", "/api/audio-upload");
    // Non-JSON Content-Type routes the server to its raw-binary branch.
    xhr.setRequestHeader("Content-Type", file.type || "application/octet-stream");
    xhr.setRequestHeader("X-Session-Id", encodeURIComponent(state.sessionId));
    xhr.setRequestHeader("X-File-Name", encodeURIComponent(file.name));
    xhr.setRequestHeader("X-File-Type", encodeURIComponent(file.type || ""));
    ...
    xhr.addEventListener("load", () => {
      ...
      if (xhr.status >= 200 && xhr.status < 300 && data.ok !== false) {
        ...
        reject(new Error(data.message || data.error || `上传失败（${xhr.status}）`));
      }
    });
